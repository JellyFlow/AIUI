...
在 AIUI 中，视图层是把意图变得可见、可操作的那一层。它主要由 WXML (WeiXin Markup Language) 和 WXSS (WeiXin Style Sheets) 组成，结合基础组件和事件系统，构建出能够呈现意图、接收用户操作并返回反馈的高性能智能体交互界面。

## WXML (页面结构)

WXML 是一种类似于 HTML 的标记语言，用于描述页面的结构。在一套意图驱动框架中，它负责把当前状态和可执行操作结构化地呈现出来。它支持数据绑定、条件渲染、列表渲染以及模板引用。
