...
- The current detailed scope is limited to the Canvas-related runtime exposed from `ink/packages/ink-script/src/canvas` and `ink/packages/ink-script/src/wx/canvas.rs`.
...
- `rotate(angle)` expects radians at the JavaScript layer and converts them before passing to the native layer.
...
- `getTransform()` returns a `DOMMatrixReadOnly` object from the runtime binding layer.
...
- `addPath(path, transform?)` throws when `transform` is present but is not a DOM matrix object accepted by the runtime parser.
- `toString()` returns SVG path data.

## 8. Authoring Rules For Agents
